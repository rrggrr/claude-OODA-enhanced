...
Do not stop to ask how many loops to orchestrate - follow the loop budget the
decision loop set (or the user named), and keep the capability read current for
as long as the loop runs.

**Inventory once, then delta-check.** Build the full catalog (Step 1) on the
first loop only. On every later loop, run a ten-second delta check instead of a
full re-inventory - it keeps the read honest without paying the inventory cost
each cycle:
...
```
CAPABILITY READ - Loop N / BUDGET  (phase: <current OODA phase>) [full | delta]
```
...
One rule specific to Observe: pick at least one channel that can reveal
**absence**, not just presence - historical logs, metrics over time, change
records, audit trails. Search-style tools only return what exists; the
decision loop's negative-space scan (the signal that *should* be there and
isn't) needs a channel with a baseline to compare against.
